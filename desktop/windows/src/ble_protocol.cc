...
ByteVector BleProtocol::GatewaySelectTargetPayload(bool self) {
    const std::string json = self
        ? "{\"event\":\"gateway_select_target\",\"self\":true}"
        : "{\"event\":\"gateway_select_target\",\"clear\":true}";
    return ByteVector(json.begin(), json.end());
}
