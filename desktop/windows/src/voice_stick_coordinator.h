...
public:
    ...
    // 点按折叠停止击的 detach 点击延迟/间隔（见 SetWechatDetachClickDelay）。
    // 生产 1500ms：detach 从请求到完成约 1.8s，按此间隔连点两次（末次加倍等待）
    // 保证浮窗关闭（2026-09-12 真机复验）。须在 Start 前调用；测试注入 0。
...
private:
    ...
    // 点按折叠停止击的 detach 点击延迟/间隔（见 SetWechatDetachClickDelay）。
    // 生产 1500ms（末次点击加倍等待，覆盖 detach 链 ~1.8s）。
