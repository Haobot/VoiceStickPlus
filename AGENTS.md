...
本文件是仓库根目录 `CLAUDE.md` 的同源副本，面向通用 AI 编码助手，内容（项目概览、构建命令、代码架构）基本一致，以 `CLAUDE.md` 为权威源。修改整体性内容时同步更新 `CLAUDE.md` 与 `CODEBUDDY.md`，避免三份文档漂移。
...
当前版本：`2.3.3`（见仓库根目录 `VERSION`）。发布前需确保 `firmware/version.txt` 与 `VERSION` 一致（当前两者均为 `2.3.3`）。
...
`CHANGELOG.md` 是版本变更记录（最新发布条目为 v2.3.3）。发布新版本时应同步追加条目；注意该文件可能滞后于 `VERSION`（中间版本 v2.0.0/v2.1.0 条目缺失，条目从 v1.9.0 直接跳到 v2.1.1），以 `VERSION`（当前 `2.3.3`）为准。
