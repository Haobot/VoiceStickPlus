...
// P2 由 3.0 下调到 1.5：依赖已有的 jerk 静止判据（STILL_JERK_DPS）+ 静止帧归零 +
// 零偏 EMA 自愈三重兜底防漂移（已在 air-mouse-still-tuning.md 论证可行），
// 换取更顺滑的低端精细对位。桌面端 low_factor 同步由 0.15 升到 0.25 补偿死区下调，
// 避免"想微动却完全不动"（见 air_mouse_kin.h AirMouseCurveParams）。
#define AIR_MOUSE_DEADZONE_DPS 1.5f
