...
// 会话数据看门狗（已停用）：排查期曾用 30s 重握手 GET_CAPS + 65s bounce 自愈，
// 但对照定案——07:04 按键正常时段零主机准备（ATVV 发现挂死、GET_CAPS 从未发），
// 小米反而持续推送；周期性 TX 写/会话重置疑似打断其输入推送状态机（真机证据：
// 会话链全就绪的固件上按键零推送）。故禁用重握手，仅保留发现阶段看门狗。
#define SESSION_IDLE_REHANDSHAKE_MS 0
#define SESSION_IDLE_BOUNCE_MS 0
...
static void tick_cb(struct ble_npl_event *ev) {
    ...
    // 会话数据看门狗已停用（见宏注释）：周期性重握手/bounce 疑似干扰小米推送。
    gateway_atvv_action_t actions[GATEWAY_ATVV_MAX_ACTIONS];
