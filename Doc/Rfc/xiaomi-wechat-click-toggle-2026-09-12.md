...
- 停止击：SendUp → **「新按住提交」循环（SendDown+repeat 1.5s → SendUp）→ 三次鼠标左键点击（间隔 1.5s/1.5s/3s，当前位置）**触发 detach 关闭。多次点击兜底：detach 从 `detach_request` 到 `detach_complete` 约 1.8s，早于完成时刻的点击不生效（真机日志实证），空会话尤其需要补点击。机制：WeType 的 commit 只在「松开时语音仍活跃（或刚停 <~1s）」时触发，点击折叠停止击天然晚于语音结束（SendUp 不 commit，文字滞留 composition）——而一次**新按住会 commit 当前 composition（commit_after_end）并重开新会话**（真机日志实证），detach 随后关闭重开的空会话；与用户亲手点击同语义（WeType 会把点击回放给光标下的应用），且 detach 必须晚于 commit（提前 = 取消并清除未上屏文本）。
