...
- 停止击收尾序列（最终形态）：SendUp → **新按住 1.5s（SendDown+repeat）→ SendUp → 1.5s → 注入左键点击 detach → 再等 1.5s → 补一次左键点击**。两次点击是必需的兜底：新按住重开的**空会话（尚无 composition）**上首次 detach 常停在 `mode=detaching` 不完成、面板残留（真机复验：用户手动再点一次即稳定关闭，且不再自弹——repeats 已停）。有音频/已提交 composition 的会话上 detach 正常 `detach_complete`。机制链：WeType 的 commit 只在「松开时语音仍活跃（或刚停 <~1s）」时触发（点击折叠停止击天然晚于语音结束 → SendUp 不 commit，文字滞留 composition 直到被 detach `detach_clear_preflight` 清除）；而**一次新按住会 commit 当前 composition（`commit_after_end`，真机日志实证）并重开新会话**——新按住即提交，detach 随后关闭重开的空会话（detach_complete 正常）。注入点击与用户亲手点击同语义（含点击回放副作用）。
