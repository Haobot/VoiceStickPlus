...
方案 A（小米 click toggle + 按住式 WeType）的「本机麦直通 + auto_switch + CABLE 渲染」管道已废弃：keyup 后 600ms 拆除管道会卡死 WeType TSF 宿主（面板不退、文字不上屏、之后新会话连 `start_received` 都没有——判据：WeType 两进程诊断日志同时停笔）。修订为直连默认麦克风：本端零采集/零切换/零渲染，WeType 弹框后直接采默认录音设备（真实麦克风）。二修（09-12 定案）：**WeType 语音热键只能启动/重启会话，keyup/ESC 均无收尾作用，面板唯一关闭路径是鼠标点击 detach**——启动击按住流限时 2500ms 自动松开（持续 repeats 会把任何关闭动作立即重新弹开面板，是「面板永不消失」元凶），停止击 = SendUp + 注入一次左键点击（光标位置）触发 detach；文字上屏由 WeType VAD 自动 commit（停说 ~150ms），与停止击解耦。StickS3 BLE 音频仍走 CABLE 管道。详见 `Doc/Expe/wetype-finalize-wedge-cable-teardown-2026-09-11.md` 追加节。
...
- 方案 A 最终修订（2026-09-11/12）：小米 click/hold 会话**直连默认麦克风**——本端零采集/零 auto_switch/零 CABLE 渲染；**WeType 语音热键只能启动/重启会话，keyup/ESC 无收尾作用，面板唯一关闭路径=鼠标点击 detach**——启动击按住流限时 2500ms 自动松开（持续 repeats 会把任何关闭动作立即重新弹开，是「面板永不消失」元凶），停止击=SendUp+注入一次左键点击（光标位置），文字上屏由 WeType VAD 自动 commit（停说 ~150ms）。旧管道「keyup 后拆除」曾永久卡死 WeType TSF 宿主（判据：两进程诊断日志同时停笔、无 `terminated composition`/无 `abort reason=`；TSF 宿主 pid=焦点应用进程，卡死只影响该应用，换记事本可隔离验证）。给第三方语音输入法供音优先直接采真实设备（物理同构），CABLE 绕行只在音频源头非 Windows 设备（BLE 流）时必要。详见 `Doc/Expe/wetype-finalize-wedge-cable-teardown-2026-09-11.md`。
