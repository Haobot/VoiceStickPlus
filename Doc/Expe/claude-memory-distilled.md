...
- 方案 A 最终修订（2026-09-11）：小米 click/hold 会话**直连默认麦克风**——本端零采集/零 auto_switch/零 CABLE 渲染，停止只 SendUp。旧管道「keyup 后拆除」会永久卡死 WeType TSF 宿主（判据：WeType 语音服务与 TSF 宿主两进程诊断日志同时停笔、无 `terminated composition`/无 `abort reason=`；TSF 宿主 pid=焦点应用进程，卡死只影响该应用，换记事本可隔离验证）。给第三方语音输入法供音优先直接采真实设备（物理同构），CABLE 绕行只在音频源头非 Windows 设备（BLE 流）时必要。详见 `Doc/Expe/wetype-finalize-wedge-cable-teardown-2026-09-11.md`。
