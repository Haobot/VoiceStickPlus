...
> **后续（2026-09-19）**：本节讲的是「重启后回连慢」；「重启后订阅假成功、语音静默失效
> 且永远恢复不了」是同一族问题的另一形态，根治见
> `Doc/Expe/ble-zombie-self-heal-2026-09-19.md`（连接期活性证明 + 僵尸后主动直连 +
> 有界自愈梯度，并定下**自愈路径绝不 unpair** 的红线）。
