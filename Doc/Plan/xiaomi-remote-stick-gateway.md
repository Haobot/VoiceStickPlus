...
| 2 | 语音链路（`xiaomi_atvv_client`+`gateway_audio`+仲裁器） | 5-8 天 | **Windows 语音端到端可用 ✅ 2026-09-18**；Mac 待验；时延增量待测 |
| 3 | 切换器 + 目标管理 + 菜单 UX | 3-5 天 | 双目标切换 ≤2s，往返稳定 |
| 4 | 打磨 + 全链路真机验收 + 文档收尾 | 2-3 天 | §7 验收清单全绿 |

### 8.1 Phase 2 + P1 桌面侧交付清单

> **Phase 2 真机验收（Windows，2026-09-18）✅**：在「Windows 蓝牙与 VS-53A8 完成系统级配对」
> 且「VoiceStick app 同时连上」的条件下，**小米遥控器的语音输入与全部按键同时正常工作**。
> 收官提交：`81d91195`（固件）、`e0db7c63`（桌面端）、`1312c925`/`4842fb41`（文档）。
> 验收过程中的 5 处根因见 §6.3 问题 8-12；经验沉淀见 `Doc/Expe/xiaomi-gateway-voice-key-and-hid-passthrough-2026-09-18.md`。
> **前置条件不可省**：HID 按键直通依赖系统级配对（§6.3 问题 12）；语音依赖 app 会话健康，
> 设备重启后若出现僵尸会话会由托盘气泡提示重新配对。P1 按键自定义真机验收仍未执行。
