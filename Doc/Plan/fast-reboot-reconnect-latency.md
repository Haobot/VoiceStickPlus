...
`kReconnectSettleDelay` 4500ms → **1500ms**（真机实测确认为终值：7 个正常
节奏样本全部一次成功，未撞僵尸，无需上调）。
...
实现提交：`54d10c6` + `e0d9e8d`（安定窗+免退避）、`7c06db6`（订阅超时）、
`e1cc940`（免退避加固）。详细数据见
`Doc/Expe/ble-zombie-link-reboot-reconnect.md` 的"二轮优化"节。

- 正常节奏单次重启：广告→已连接 4.6-6.1s，中位 **5.4s**（一轮优化前 ~6.7s）。
  未达到预期的 3.5-4s，差距主要来自两处低估：扫描/认领延迟 ~0.5-2s，
  以及 `GattServicesChanged` 每次连接都使 GATT 缓存失效、cached 服务发现
  实际要走空口 ~760ms。剩余均为硬成本，不再继续压缩。
- 连按风暴（秒级连续重启）：2.5s 超时 + 免退避重试按设计兜底，~6.5-7.5s
  自愈；观察到双僵尸病理案例（一轮 23s）后按实测加固——**免退避从消费一次
  扩为 15s 窗内最多 3 次**（`kZombieSuspectWindow` /
  `kZombieSuspectMaxFreeRetries`，`e1cc940`）。
- 固件 boot→广播 ~1s（主机日志反推），<1s 阈值附近，固件不动。
- `winrt::when_any` 要求所有分支同类型（static_assert），实现时把订阅操作
  包了一层返回 `IAsyncAction` 的 IILE 再与定时器竞速，语义与设计一致。
