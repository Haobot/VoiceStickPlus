...
The manifest is generated by the release workflow from `releases/latest/download/manifest.json` (the Pages copy under `/firmware/` rewrites `merged_url` to the same-origin address). Fields:
...
| `ota_url` / `ota_sha256` / `ota_size` | App-slot image for BLE OTA, with SHA-256 and byte size for verification. |
| `merged_url` / `merged_sha256` / `merged_size` | Merged 0x0 image for USB flashing, with checksums. |
