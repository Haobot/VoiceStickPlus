...
- **网关切换器验收**：`gateway_switch_acceptance.py` 一条命令跑完 P1 切换验收——自己采串口（保持 DTR/RTS 低不扰动设备）+ 解析 app 日志，
  逐项判 PASS/FAIL：设备侧切换 ≤2s、切换期间小米链路零断开、订阅时 MTU 截断告警为 0、目标机 app 恢复时间（记录值），
  以及人手阶段（长按唤起菜单/旋转/短按确认）的判定依据。用法与判据见 `Doc/Plan/xiaomi-gateway-p1-switcher.md` §4.5。
  注意：**设备无法被第三方 central 驱动**（app 退出后 Windows 会用系统级 HOGP 配对自动连走设备并停止广播），
  所以本轮验收走桌面端入口 `VoiceStick.exe --gateway-target self|clear` 而不是 bleak。
